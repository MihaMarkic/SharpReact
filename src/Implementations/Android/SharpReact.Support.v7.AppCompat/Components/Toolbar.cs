...
namespace SharpReact.Support.v7.AppCompat.Components
{
	public  partial class Toolbar<TProps, TElement>: ViewGroup<TProps, TElement>
		...
		where TElement : global::Android.Support.V7.Widget.Toolbar
	{
		public override void CreateElement(global::Android.Content.Context context)
		{
			Element = (TElement)new global::Android.Support.V7.Widget.Toolbar(context);
			InitElement(Element);
		}
		...
		static void UpdateToolbarWithInstanceProperties(TElement element, TProps previous, TProps nextProps)
		{
			...
			if (!(previous?.NavigationClick is null) && nextProps.NavigationClick is null)
			{
				element.NavigationClick -= nextProps.NavigationClick;
			}
			if (previous?.NavigationClick is null && !(nextProps.NavigationClick is null))
			{
				element.NavigationClick += nextProps.NavigationClick;
